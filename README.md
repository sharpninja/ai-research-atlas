...
Every entry has Cited By and Citations sections in the publication sidebar below the original source record, including the extended AI Toy page. Both directions are derived from `citation-links.json`, a single reviewed graph of 156 relationships. The original 24 curated links and their publication-version notes are retained. Unknown entries, duplicate edges, self links, or missing source evidence fail the build.

`bibliography-index.json` registers all 67 works: 65 indexed reference sections or bibliographical-footnote records, one partial record (MYCIN), and one unavailable bibliography (Smith and Zipser). Complete coverage of every original edition has not yet been established. MYCIN has 26 publisher-deposited records under 25 bibliography keys, with missing fields; the original reference pages remain necessary. Smith and Zipser's publisher page has no HTML reference list and restricts access to the paper.

The recovery includes Hebb's 15 bibliography pages, Quillian's ten pages, all 147 references in Associative Memory, all 22 reference pages in Content-Addressable Memories, TLC's 22 references, Nature's four temporal-recall references, Little's twelve references, the Logic Theory Machine's bibliographical footnotes, and the AI Toy Sources box. Elman's record explicitly indexes the 1990 journal replacement designated by the author, not the unavailable 1988 report. Perceptrons uses the 1988 edition retaining the original text with 1972 corrections; later handwritten references are separated and uncertain metadata is labeled.

STUDENT, focused backpropagation, SHRDLU, Hebb, Semantic Memory, Elman's 1990 paper, and the AI Toy source box include OCR text with PDF page numbers and extraction hashes. OCR and PDF extraction may retain spelling or column-order errors; raw text alone does not authorize a citation link. Publication versions are stated in each record's notes and `BibliographyEdition` field where relevant.
