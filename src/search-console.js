...
export async function searchConsoleSummary(env, days, fetcher = fetch) {
  ...
  const response = await fetcher(endpoint, {method:'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({startDate:date(start),endDate:date(end),dimensions:['query','page'],type:'web',dataState:'all',rowLimit:1000})});
